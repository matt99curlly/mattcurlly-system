...
def get_drug_list():
    try:
        ...
        return ["Aspirin", "Ibuprofen", "Alcohol", "Diazepam"] # Fallback
...
def get_risk_info(d1, d2):
    ...
    if d1 in nsaids and d2 in nsaids:
        return {"risk": "High", "effect": "Stomach bleeding.", "rec": "Avoid mixing."}
...
async def home(request: Request):
    return templates.TemplateResponse("index.html", {"request": request, "drugs": get_drug_list()})

@app.post("/check", response_class=HTMLResponse)
async def check(request: Request, drug1: str = Form(...), drug2: str = Form(...)):
    result = get_risk_info(drug1, drug2)
    return templates.TemplateResponse("index.html", {
        "request": request, "drugs": get_drug_list(),
        "drug1": drug1, "drug2": drug2,
        "risk_level": result["risk"], "side_effects": result["effect"], "recommendation": result["rec"]
    })
